% receiving_q: receiving function with qmax and rhoj as inputs for
...
function R=receiving_q(rho,rhoj,qmax,vmax,numLanes,err_R,err_Q,Q_R,Q_Q,isApp)
...
if isApp==1
    if rho>=rhoc
        err=lognrnd(err_R,sqrt(Q_R));
    else
        err=lognrnd(err_Q,sqrt(Q_Q));
    end
else
    err=0;
end
