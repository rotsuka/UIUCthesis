...
linkInd=3; % hardcoded for link three
maxCap=2400; % hardcoded
...
for k=start:fin
    ...
    title(['Forward, t=',num2str((k-1)*5),', \mu=',num2str(meanVec2(count)),...
        ', \sigma=',num2str(sqrt(varVec2(count)))])
    
    set(gca,'YLim',[0 100])
    set(gca,'XTickLabel',[maxCap maxCap*.75 maxCap*.5 maxCap*0])
    
    xlabel('Qmax (veh/hr/lane)')
    ylabel(['Number of ens at t=',num2str((k-1)*5)])
    
    % Kalman filter
    
    subplot(1,3,3)
    bar(1:4,damageMat(:,k));
    title(['Kalman, t=',num2str((k-1)*5),', \mu=',num2str(meanVec(count)),...
