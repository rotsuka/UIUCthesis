...
% computing in MATLAB
%
% In this function, variables with a "_wEQ" ending mean they are for the
...
% % the function should be fine, but pass back RG.valueNodes just in case
% RG.numNodes=5;
% [RG.valueNodes,RG.keyNodes]=inpNodes(RG.numNodes);
% 
% % Links
% 
% % Pass back the cell of links instead of the actual map so multiple map
% % object with difrerent handles can be created
% RG.numLinks=6; % internal/boundary combined total
% [RG.valueLinks,RG.keyLinks]=inpLinks(RG.numLinks);
% 
% % Sensors
% 
% %NOTE: mapSensors does not need to be copied so passing the map back through
% % the function should be fine, but pass back RG.valueSensors just in case
% RG.numSensors=2;
% [RG.valueSensors,RG.keySensors]=inpSensors(RG.numSensors);
% 
% % EQ 
% RG.newEQ=inpEQ;
% 
% % Tell which links are actually bridges
% RG.bridgeLinks=inpBridge;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% PRELOADED ROAD GEOMETRY

% RG=load('Geom_4link5node_2Sens_SensNoise5_MD.mat');
RG=load('Geom_4link5node_2Sens_SensNoise5_HI.mat');
% RG=load('Geom_4link5node_2Sens_SensNoise5_TO.mat');
...
mapLinksNoise=containers.Map(RG.keyLinks,RG.valueLinks);
...
% nRG.valueLinks=alterParams(RG.valueLinks);
% mapLinksApp_wEQ=containers.Map(RG.keyLinks,nRG.valueLinks); % filter, EQ
% mapLinksApp2_wEQ=containers.Map(RG.keyLinks,nRG.valueLinks); % open loop, EQ
% mapLinksApp_nEQ=containers.Map(RG.keyLinks,nRG.valueLinks); % filter, no EQ
% mapLinksApp2_nEQ=containers.Map(RG.keyLinks,nRG.valueLinks); % open loop, no EQ

mapLinksApp_wEQ=containers.Map(RG.keyLinks,RG.valueLinks); % filter, EQ
mapLinksApp2_wEQ=containers.Map(RG.keyLinks,RG.valueLinks); % open loop, EQ
mapLinksApp_nEQ=containers.Map(RG.keyLinks,RG.valueLinks); % filter, no EQ
mapLinksApp2_nEQ=containers.Map(RG.keyLinks,RG.valueLinks); % open loop, no EQ

% mapNodes
mapNodes=containers.Map(RG.keyNodes,RG.valueNodes);

% mapSensors (ONLY TRAFFIC DENSITY SENSORS)
mapSensors=containers.Map(RG.keySensors,RG.valueSensors);
...
mapLinksNoise=getLengthCoord(mapLinksNoise,mapNodes,RG.numLinks);

mapLinksApp_wEQ=getLengthCoord(mapLinksApp_wEQ,mapNodes,RG.numLinks);
mapLinksApp2_wEQ=getLengthCoord(mapLinksApp2_wEQ,mapNodes,RG.numLinks);

mapLinksApp_nEQ=getLengthCoord(mapLinksApp_nEQ,mapNodes,RG.numLinks);
mapLinksApp2_nEQ=getLengthCoord(mapLinksApp2_nEQ,mapNodes,RG.numLinks);

% Compute dx,number of cells, and start/end cell for link using CFL
% condition and overwrite previous map obj.
mapLinksNoise=getCellDisc(DT,mapLinksNoise,RG.keyLinks,RG.numLinks);

mapLinksApp_wEQ=getCellDisc(DT,mapLinksApp_wEQ,RG.keyLinks,RG.numLinks);
mapLinksApp2_wEQ=getCellDisc(DT,mapLinksApp2_wEQ,RG.keyLinks,RG.numLinks);

mapLinksApp_nEQ=getCellDisc(DT,mapLinksApp_nEQ,RG.keyLinks,RG.numLinks);
mapLinksApp2_nEQ=getCellDisc(DT,mapLinksApp2_nEQ,RG.keyLinks,RG.numLinks);

% The total number of cells takes into account the CTM model and the two
% ghost cells.
totCells=compTotCells(mapLinksNoise,RG.numLinks);

% Add the initial conditions and overwrite previous map obj
mapLinksNoise=addInitialCond(mapLinksNoise,RG.keyLinks,rho0Acc);

mapLinksApp_wEQ=addInitialCond(mapLinksApp_wEQ,RG.keyLinks,rho0App);
mapLinksApp2_wEQ=addInitialCond(mapLinksApp2_wEQ,RG.keyLinks,rho0App);

mapLinksApp_nEQ=addInitialCond(mapLinksApp_nEQ,RG.keyLinks,rho0App);
mapLinksApp2_nEQ=addInitialCond(mapLinksApp2_nEQ,RG.keyLinks,rho0App);

% Determine which links are bridges and overwrite previous map obj
mapLinksNoise=getBridges(mapLinksNoise,RG.numLinks,RG.bridgeLinks);

mapLinksApp_wEQ=getBridges(mapLinksApp_wEQ,RG.numLinks,RG.bridgeLinks);
mapLinksApp2_wEQ=getBridges(mapLinksApp2_wEQ,RG.numLinks,RG.bridgeLinks);

mapLinksApp_nEQ=getBridges(mapLinksApp_nEQ,RG.numLinks,RG.bridgeLinks);
mapLinksApp2_nEQ=getBridges(mapLinksApp2_nEQ,RG.numLinks,RG.bridgeLinks);

% Attach links onto nodes and overwrite previous map obj
mapNodes=attachLinks(mapNodes,RG.numNodes,mapLinksApp_wEQ,RG.numLinks);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% MEASUREMENT MATRIX, H

% Rearrange sensor order and give a global cell position. RG.keySensors here
% gives the keys back in the desired order.
[mapSensors RG.keySensors sortInd]=getSensorPos(mapSensors,mapLinksApp_wEQ,...
    RG.numSensors);

% Attach sensors onto respective links. No need to attach sensors to open
% loop map objects.
mapLinksApp_wEQ=attachSensors(mapLinksApp_wEQ,mapSensors,RG.numSensors,RG.keySensors);
mapLinksApp_nEQ=attachSensors(mapLinksApp_nEQ,mapSensors,RG.numSensors,RG.keySensors);

% Get measurement matrix, H and its dimensions, kk and n
[H kk n]=getHSens(mapSensors,RG.keySensors,totCells,RG.numSensors);
...
x0Noise=getStates0(mapLinksNoise,totCells,RG.numLinks);
...
mapLinksNoise=getDistanceEQ(mapLinksNoise,RG.numLinks,RG.newEQ);

% Construct vector of distances
distVecN=getDistVec(mapLinksNoise,RG.keyLinks,RG.bridgeLinks,RG.numLinks);

% Get peak ground accelerations
PGAN=getPGA(RG.newEQ.mag,distVecN,F,S_SR,S_HR);
...
for k=1:length(measTime)-1
    ...
    if k==keq
        ...
        mapLinksNoise=getMapParamsN(mapLinksNoise,RG.numLinks,RG.keyLinks,DSlimsN);
...
% densPlotter_one(xMatNoise,mapLinksNoise,RG.numLinks,measTime);
...
noisyMat=getNoisyMat(mapSensors,RG.numSensors,RG.keySensors,measTime);

% Get the measurement vector by adding the noisyMat to the true solution 
y=getyMeas(H,xMatNoise,measTime,RG.numSensors,noisyMat);
...
qmaxArray_wEQ=zeros(RG.numLinks,1,noEn,K);

qmaxArray_nEQ=zeros(RG.numLinks,1,noEn,K);

% Averaged measurement noise
v_bar=zeros(RG.numSensors,1);
...
% Initial states
xk_p_wEQ(1:totCells,1,1)=getStates0(mapLinksApp_wEQ,totCells,RG.numLinks);
xk_p_nEQ(1:totCells,1,1)=getStates0(mapLinksApp_nEQ,totCells,RG.numLinks);
...
qmaxArray2_wEQ=zeros(RG.numLinks,1,noEn,K);
...
qmaxArray2_nEQ=zeros(RG.numLinks,1,noEn,K);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% If you dont do this overwriting, qmax and rhoj will both be 0, which will
% cause distDraw0 to go in an infinite loop.
mapLinksApp_wEQ=setInitialParams(mapLinksApp_wEQ,RG.numLinks);
mapLinksApp2_wEQ=setInitialParams(mapLinksApp2_wEQ,RG.numLinks);
mapLinksApp_nEQ=setInitialParams(mapLinksApp_nEQ,RG.numLinks);
mapLinksApp2_nEQ=setInitialParams(mapLinksApp2_nEQ,RG.numLinks);
...
for i=1:noEn
    ...
    [xi_p_wEQ(1:totCells,1,i,1) redrawCount]=distDraw0(xk_p_wEQ(:,1,1),Pk_p_wEQ(:,:,1),mapLinksApp_wEQ,...
        RG.numLinks,totCells,redrawCount,i);
...
for k=1:K-1
    ...
    for i=1:noEn
        ...
        if k>=keq
            ...
            [m_wEQ xloc_wEQ yloc_wEQ]=getEQParams(1,RG.newEQ);
            [m_nEQ xloc_nEQ yloc_nEQ]=getEQParams(0,RG.newEQ);
            ...
            % mapLink objects.
            mapLinksApp_wEQ=getDistanceEQ(mapLinksApp_wEQ,RG.numLinks,nEQ_wEQ);
            mapLinksApp2_wEQ=getDistanceEQ(mapLinksApp2_wEQ,RG.numLinks,nEQ_wEQ);
            
            mapLinksApp_nEQ=getDistanceEQ(mapLinksApp_nEQ,RG.numLinks,nEQ_nEQ);
            mapLinksApp2_nEQ=getDistanceEQ(mapLinksApp2_nEQ,RG.numLinks,nEQ_nEQ);
            
            % Construct vector of distances, R. Only need one set of
            % params for wEQ and one set of params for nEQ. Open loop or
            % filter does not matter.
            distVec_wEQ=getDistVec(mapLinksApp_wEQ,RG.keyLinks,RG.bridgeLinks,RG.numLinks);
            distVec_nEQ=getDistVec(mapLinksApp_nEQ,RG.keyLinks,RG.bridgeLinks,RG.numLinks);
            ...
            % ensemble)
            mapLinksApp_wEQ=getMapParams(mapLinksApp_wEQ,RG.numLinks,RG.keyLinks,DSlims_wEQ);
            mapLinksApp2_wEQ=getMapParams(mapLinksApp2_wEQ,RG.numLinks,RG.keyLinks,DSlims_wEQ);
            
            mapLinksApp_nEQ=getMapParams(mapLinksApp_nEQ,RG.numLinks,RG.keyLinks,DSlims_nEQ);
            mapLinksApp2_nEQ=getMapParams(mapLinksApp2_nEQ,RG.numLinks,RG.keyLinks,DSlims_nEQ);
        ...
        qmaxArray_wEQ(:,1,i,k+1)=getqmaxMat(mapLinksApp_wEQ,RG.keyLinks);
        qmaxArray2_wEQ(:,1,i,k+1)=getqmaxMat(mapLinksApp2_wEQ,RG.keyLinks);
        
        qmaxArray_nEQ(:,1,i,k+1)=getqmaxMat(mapLinksApp_nEQ,RG.keyLinks);
        qmaxArray2_nEQ(:,1,i,k+1)=getqmaxMat(mapLinksApp2_nEQ,RG.keyLinks);
    ...
    D_wEQ=zeros(RG.numSensors,noEn);
    Gam_wEQ=zeros(RG.numSensors,noEn);
    
    D_nEQ=zeros(RG.numSensors,noEn);
    Gam_nEQ=zeros(RG.numSensors,noEn);
    ...
    for i=1:noEn
        ...
        [ensMeas ensMeasNoise]=getEnsMeasNoise(meas,mapSensors,RG.keySensors);
